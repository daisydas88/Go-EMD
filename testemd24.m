...
c = s(:)'; % copy of the input signal (as a row vector)
% c=sgolayfilt(p,5,33);
% for input sine wave
% c=x;
N = length(c);
...
m=imf(:,13)';
y13 = var(m)
ivar=y1+y2+y3+y4+y5+y6+y7+y8+y9+y10+y11+y12+y13;
%mean
wholemean = mean(c)
...
n=imf(:,13)';
my13 = mean(n)
imean=my1+my2+my3+my4+my5+my6+my7+my8+my9+my10+my11+my12+my13;
...
ss=imf(:,13)';
sv13=std(ss)
isd=sv1+sv2+sv3+sv4+sv5+sv6+sv7+sv8+sv9+sv10+sv11+sv12+sv13;
...
ss=imf(:,13)';
sk13=skewness(ss)
isk=sk1+sk2+sk3+sk4+sk5+sk6+sk7+sk8+sk9+sk10+sk11+sk12+sk13;
...
title('Box plot of EMD')
%code for reconstruction signal
rs=imf(:,1)'+imf(:,2)'+imf(:,3)'+imf(:,4)'+imf(:,5)'+imf(:,6)'+imf(:,7)'+imf(:,8)'+imf(:,9)'+imf(:,10)'+imf(:,11)'+imf(:,12)'+imf(:,13)'

figure;
subplot(2,1,1);
plot(rs);xlabel('time(s)')
title('Reconstructed signal from EMD')
subplot(2,1,2);
plot(c);
xlabel('time(s)')
ylabel('freq')
...
set(gcf,'name','SSA of IMFs produced by EMD')
clf;
plot(t,c,t,sum(imf(:,:),12));
ylabel('Power')
xlabel('Frequency(Hz)')
legend('Original','Complete reconstruction');
title('SSA of IMFs produced using EMD')

figure;
%histogram plot
histogram(imf);
title('Histogram for IMFs generated by EMD')
xlabel('IMF(s)');
ylabel('Iterations');
...
%mesh plot
mesh(imf);
title('Mesh for IMFs generated by EMD')
xlabel('IMF(s)');
ylabel('Occurence');